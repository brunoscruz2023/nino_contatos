...
App.Layout.Shell = {
    ...
    init: function() {
        ...
        if (!App.Core.Security.hasModuleAccess('mapa')) {
            if (App.Core.Security.hasModuleAccess('agenda')) firstView = 'eventos';
    ...
    renderNav: function() {
        ...
        // Adiciona módulos disponíveis, ignorando o que está ativo
        if (App.Core.Security.hasModuleAccess('mapa') && this.activeView !== 'mapa') availableMods.push('mapa');
        if (App.Core.Security.hasModuleAccess('agenda') && this.activeView !== 'eventos') availableMods.push('eventos');
        if (App.Core.Security.hasModuleAccess('cadastro') && this.activeView !== 'cadastro') availableMods.push('cadastro');
        if (App.Core.Security.hasModuleAccess('admin') && this.activeView !== 'admin') availableMods.push('admin');

        // Se houver outros módulos além do atual, o FAB é contextual e o Logout vai pra barra
        if (availableMods.length > 0) {
            availableMods.push('logout');
            
            // Distribui igualmente (max 2 esquerda, max 2 direita)
            let half = Math.ceil(availableMods.length / 2);
            let leftMods = availableMods.slice(0, half);
            let rightMods = availableMods.slice(half);
            ...
            // Se só tem 1 módulo (o ativo), o FAB vira Logout
    ...
    createNavBtn: function(view) {
        ...
        else if (view === 'eventos') { icon = ICONS.eventos; label = 'Eventos'; }
    ...
    setActive: function(view) {
        ...
        // Re-renderiza a barra para ocultar o botão ativo
        this.renderNav();
    },
