...
App.Mapa.Mobile = {
    getMobileIndicator: function(delta, count) {
        var arrow = ''; var colorClass = 'text-blue-500';
        if (delta > 0) { arrow = '↑'; colorClass = 'text-emerald-500'; } 
        else if (delta < 0) { arrow = '↓'; colorClass = 'text-rose-500'; }
    ...
    toggleSubzona: function(regionName, subzonaName) {
        ...
        if (expandedSubzonas.has(key)) expandedSubzonas.delete(key);
        else expandedSubzonas.add(key);
    ...
    initMobileList: function() {
        ...
        geoDatabase.forEach(function(data) {
            ...
            var cardHtml = App.UI.AccordionList.createCard({
                title: data.bairro, badge: '0', badgeLabel: 'leads', uiColor: uiColor, isCollapsible: temAcesso
            });
        ...
        if (useGrouping) {
            ...
            subzonas.forEach(function(sz) {
                ...
                wrapper.innerHTML = '<div class="mobile-lead-card bg-white p-4 rounded-2xl border border-slate-100 shadow-sm cursor-pointer" onclick="App.Mapa.Mobile.toggleSubzona(\'' + currentRegionFilter + '\', \'' + safeName + '\')">' +
                    '<div class="flex items-center gap-4">' +
                        '<div class="w-10 h-10 rounded-xl ' + uiColor.dot + ' bg-opacity-10 flex items-center justify-center flex-shrink-0"><div class="w-3 h-3 rounded-full ' + uiColor.dot + '"></div></div>' +
                        '<div class="flex-1 min-w-0"><p class="text-sm font-bold text-slate-800 truncate">' + sz.nome + '</p><div class="flex items-center gap-1 text-[10px] font-bold mt-0.5">' + trendIcon + weekInd + '<span class="text-slate-300 mx-0">/</span>' + monthInd + '</div></div>' +
                        '<div class="text-right flex-shrink-0"><p class="text-2xl font-extrabold ' + uiColor.text + ' leading-none">' + sz.total + '</p><p class="text-[10px] text-slate-400 font-medium mt-1">leads</p></div>' +
                ...
                sz.bairros.forEach(function(b) {
                    if (b.domMobileCard) {
                        ...
                        if (countEl) { countEl.classList.remove('text-2xl'); countEl.classList.add('text-xl'); }
                    }
                });
            ...
            geoDatabase.forEach(function(data) {
                if (data.domMobileCard) {
                    ...
                    if (countEl) { countEl.classList.remove('text-xl'); countEl.classList.add('text-2xl'); }
                    if (data.isVisible) data.domMobileCard.classList.remove('is-hidden-mobile');
                    else data.domMobileCard.classList.add('is-hidden-mobile');
                }
            });
        }
    }
